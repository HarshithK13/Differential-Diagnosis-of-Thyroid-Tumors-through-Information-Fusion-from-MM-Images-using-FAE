...
parent_directory = '/home/reddy16/BSHG_EB4'
for bshg_image, label in zip(bshg_images_train, class_labels_train):
    # Convert the BSHG image to uint8 format and adjust the shape
    bshg_image = (bshg_image * 255).astype(np.uint8)
    bshg_image = np.reshape(bshg_image, (512, 512))
    ...
    Image.fromarray(bshg_image).save(os.path.join(destination_directory, unique_filename))
...
parent_directory2 = '/home/reddy16/BSHG_EB4_valid'
for bshg_image, label in zip(bshg_images_val, class_labels_val):
    # Convert the BSHG image to uint8 format and adjust the shape
    bshg_image = (bshg_image * 255).astype(np.uint8)
    bshg_image = np.reshape(bshg_image, (512, 512))
    # bshg_image = np.squeeze(bshg_image, axis=2)  # Remove the single-channel dimension
...
parent_directory3 = '/home/reddy16/BSHG_EB4_test'
for bshg_image, label in zip(bshg_images_test, class_labels_test):
    # Convert the BSHG image to uint8 format and adjust the shape
    bshg_image = (bshg_image * 255).astype(np.uint8)
    bshg_image = np.reshape(bshg_image, (512, 512))
    # bshg_image = np.squeeze(bshg_image, axis=2)  # Remove the single-channel dimension
...
# Change here: Use ResNet-18 instead of EfficientNet
...
train_dir = "/home/reddy16/BSHG_EB4"
val_dir = "/home/reddy16/BSHG_EB4_valid"
test_dir = "/home/reddy16/BSHG_EB4_test"
