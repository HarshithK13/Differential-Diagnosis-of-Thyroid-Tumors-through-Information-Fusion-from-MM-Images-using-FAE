...
if file_exists==False:
    for epoch in range(epochs):
        running_loss = 0.0
        loss_fshg, loss_tpef= 0.0,0.0
        for fshg,tp, img_folder, class_label in zip(fshg_images_train,tpef_images_train, train_data_roi, class_labels_train):
...
#             print("shape1 of tpef:",tp.shape)
            ...
            re_fshg,re_tp, latent_image = model(fshg, tp)
...
#             print("shape1 of re_tp:",re_tp.shape)
        ...
        with torch.no_grad():
            ...
            running_loss = 0.0
            loss_fshg, loss_tpef = 0.0,0.0
            for fshg, tp, img_folder, class_label in zip(fshg_images_val, tpef_images_val, valid_data_roi, class_labels_val):
...
output_loss_test = []
with torch.no_grad():
    for fshg, tp, img_folder, class_label in zip(fshg_images_test, tpef_images_test, test_data_roi, class_labels_test):
