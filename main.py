...
# train_data = ['541930','541925','541938', '541937', '693345','568151','568149',
#                 '568148','494918','568146','568145','693348','540888','540887','540886','494922',
#                 '494921','494920'] #2

train_data = ['541937','541938','541952', '541951', '693345','693346','568149',
                '693348','494918','494917','568145','540889','494919','540887','693347','494922',
                '494921','494920'] #3
# train_data = ['541938','541951','541952', '542858', '494917','494922','693347',
#                 '693346','568147','540887','693348','540889','540888','494918','540886','494917',
#                 '568149','494920'] #4



# valid_data = ['541952', '541951', '494919', '693347', '693346'] #1
# valid_data = ['542871', '542858', '540889', '568147', '494917'] #2
valid_data = ['541925', '542871', '540888', '540886', '568151'] #3
# valid_data = ['541925', '541937', '568151', '568145', '568148'] #4
...
# test_data = ['541930','541925','693348','494918','494917'] #setting1
# test_data = ['541952','541951','494919','693347','693346'] #setting2
test_data = ['541930','542858','568148','568147','568146'] #setting3 
# test_data = ['542871','541930','693345','568146','494921'] #setting4
...
class AE(torch.nn.Module):
    def __init__(self):
        ...
        self.t_conv7 = nn.ConvTranspose2d(4, 1, 3)
        self.t_conv8 = nn.ConvTranspose2d(1, 1, 3)
    ...
    def forward(self, b,f,t):
        
        stack_img = np.zeros(shape=(1,12,502,502), dtype=np.float32)
        stack_img = torch.tensor(stack_img).to(device)
        

        b1 = F.gelu(self.bn0(self.conv0(b)))
        f1 = F.gelu(self.conv0(f))
        ...
        print("Shape after conv2:", x.shape)            # added layer
        ...
        print("Shape after t_conv10:", x3.shape)

        x1 = F.gelu(self.t_conv11(x1))
        x2 = F.gelu(self.t_conv11(x2))
        x3 = F.gelu(self.t_conv11(x3))
        print("Shape after t_conv11:", x3.shape)

        # x3 = F.gelu(self.t_conv12(x3))
        # x3 = F.gelu(self.t_conv12(x3))
        # x3 = F.gelu(self.t_conv12(x3))
        # print("Shape after t_conv12:", x3.shape)
...
if file_exists==False:
    for epoch in range(epochs):
        running_loss = 0.0
        loss_bshg, loss_fshg, loss_tpef = 0.0,0.0,0.0
        for bshg,fshg,tp,img_folder, class_label in zip(bshg_images_train, fshg_images_train, tpef_images_train,train_data_roi, class_labels_train):
...
#             print("shape1 of tpef:",tp.shape)
            ...
            re_bshg,re_fshg,re_tp,latent_image = model(bshg,fshg,tp)
...
#             print("shape1 of re_tpef:",re_tp.shape)
            ...
            loss = loss1 + 0.5*loss2 + 0.05*loss3        
        ...
        with torch.no_grad():
            ...
            running_loss = 0.0
            loss_bshg, loss_fshg, loss_tpef = 0.0,0.0,0.0
            for bshg, fshg, tp,img_folder, class_label in zip(bshg_images_val, fshg_images_val, tpef_images_val,valid_data_roi, class_labels_val):
...
#                 print("shape1 of re_tpef:",re_tp.shape)
                ...
                loss = loss = loss1 + 0.5*loss2 + 0.05*loss3
...
output_loss_test = []
with torch.no_grad():
    for bshg, fshg, tp,img_folder, class_label in zip(bshg_images_test, fshg_images_test, tpef_images_test,test_data_roi, class_labels_test):
