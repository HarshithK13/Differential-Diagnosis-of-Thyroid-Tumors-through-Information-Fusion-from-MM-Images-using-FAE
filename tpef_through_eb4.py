...
parent_directory = '/home/reddy16/TPEF_EB4'
for tpef_image, label in zip(tpef_images_train, class_labels_train):
    # Convert the BSHG image to uint8 format and adjust the shape
    tpef_image = (tpef_image * 255).astype(np.uint8)
    tpef_image = np.reshape(tpef_image, (512, 512))
    ...
    Image.fromarray(tpef_image).save(os.path.join(destination_directory, unique_filename))
...
parent_directory2 = '/home/reddy16/TPEF_EB4_valid'
for tpef_image, label in zip(fshg_images_val, class_labels_val):
    # Convert the BSHG image to uint8 format and adjust the shape
    tpef_image = (tpef_image * 255).astype(np.uint8)
    tpef_image = np.reshape(tpef_image, (512, 512))
    # bshg_image = np.squeeze(bshg_image, axis=2)  # Remove the single-channel dimension
...
parent_directory3 = '/home/reddy16/TPEF_EB4_test'
for tpef_image, label in zip(tpef_images_test, class_labels_test):
    # Convert the BSHG image to uint8 format and adjust the shape
    tpef_image = (tpef_image * 255).astype(np.uint8)
    tpef_image = np.reshape(tpef_image, (512, 512))
    # bshg_image = np.squeeze(bshg_image, axis=2)  # Remove the single-channel dimension
...
train_dir = "/home/reddy16/TPEF_EB4"
val_dir = "/home/reddy16/TPEF_EB4_valid"
test_dir = "/home/reddy16/TPEF_EB4_test"
